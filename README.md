...
- **Модульная архитектура провайдеров (`SourcePluginRegistry`)**:
  - `AnimelibPlugin` — нативный плеер AnimeLib с резолвом прямых потоков вплоть до 4K (2160p) и 2K (1440p), HLS/m3u8-плейлистов и динамической приоритизацией студий озвучки.
  - `KodikPlugin` — резервный внешний плеер для надежного воспроизведения и скачивания альтернативных озвучек.
  - Автоматическая регистрация плагинов через `registerSourcePlugin` с возможностью легкого расширения.
- **Интеллектуальный Telegram-бот (`grammY`)**:
  - Обязательный интерактивный выбор качества видео (от 360p до 4K 2160p Ultra HD) и источника (AnimeLib, Kodik 1080p) для каждой серии без принудительного автоскачивания.
  - Умная очередь скачивания (`downloadManager`) с защитой от OOM через Node.js HTTP-стримы (`axios` + `stream.pipeline` / `createWriteStream`).
  - Быстрое переключение отслеживания новых серий кнопкой `[🔔 Отслеживается]` / `[🔕 Не отслеживать]` прямо в Telegram с сохранением в SQLite.
  - Раздел «Запланированное» с фильтрацией по активному колокольчику, мгновенным фолбэком на полный список и постраничной навигацией.
  - Индивидуальная настройка озвучек по каждому тайтлу и глобальный список приоритетных студий.
- **Гостевой режим и устойчивость к сети**:
  - Полная работоспособность бота при отсутствии, отзыве или истечении токенов Shikimori: бот автоматически переходит в локальный режим `AnimeLib ➔ SQLite [Локальный режим ⏸]` и продолжает трекинг и скачивание серий.
  - Автоматический 45-секундный кеш запросов закладок и SQLite-fallback при сбоях внешней сети.
...
Подробный журнал всех версий и изменений оформлен в [CHANGELOG.md](CHANGELOG.md) (формат Keep a Changelog).
Предыдущие сводки релизов также доступны в [CHANGES.md](CHANGES.md).
